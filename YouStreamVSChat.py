...
try:
    ...
    print("❌ Error: Could not import YouStream_VSChat_KeyCodes.py")
...
DONATION_TIERS = {
    1: {"multiplier": 1, "color": "🟢", "name": "Mini Don", "color_hex": "#00FF00"},
...
COMMAND_CONFIG = {
    "cmd1": {"key": NUMPAD_0, "command": "!action1", "enabled": True},
    "cmd2": {"key": NUMPAD_1, "command": "!action2", "enabled": True},
    "cmd3": {"key": NUMPAD_2, "command": "!action3", "enabled": True},
    "cmd4": {"key": NUMPAD_3, "command": "!action4", "enabled": True},
    "cmd5": {"key": NUMPAD_4, "command": "!action5", "enabled": True},
    "cmd6": {"key": NUMPAD_5, "command": "!action6", "enabled": True},
    "cmd7": {"key": NUMPAD_6, "command": "!action7", "enabled": True},
    "cmd8": {"key": NUMPAD_7, "command": "!action8", "enabled": True},
    "cmd9": {"key": NUMPAD_8, "command": "!action9", "enabled": True},
    "cmd10": {"key": NUMPAD_9, "command": "!action10", "enabled": True},
    "cmd11": {"key": NUMPAD_PLUS, "command": "!action11", "enabled": True},
    "cmd12": {"key": NUMPAD_MINUS, "command": "!action12", "enabled": True},
    "cmd13": {"key": NUMPAD_MULTIPLY, "command": "!action13", "enabled": True},
    "cmd14": {"key": NUMPAD_PERIOD, "command": "!action14", "enabled": True},
}
...
class SetupDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        subtitle = QLabel("• Connection Settings •")
        ...
        instructions = QLabel(
            "---------------------Instructions----------------------\n"
            "• YouTube: Video ID (11 characters after 'v=')\n"
            "• Twitch: Channel name (all lowercase)\n\n"
            "---------------------Data----------------------\n"
            "• Super Chats/Bits will activate multipliers!\n"
...
class CommandConfigDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        for cmd_key, config in self.command_config.items():
            group = QGroupBox(config["command"]) 
            ...
            command_edit.setStyleSheet(f"""
                ...
                border-radius: 3px;
            """)
            command_edit.textChanged.connect(lambda text, c=cmd_key: self.update_command(c, text))
    ...
    def update_command(self, cmd_key, command_text):
...
class MainWindow(QMainWindow):
    def __init__(self, platform, video_id="", twitch_channel=""):
        ...
        self.chat_timer.start(int(MESSAGE_RATE * 1000))
        ...
        if not self.simulation_mode:
            ...
            self.connection_label.setText("🎮 Test Mode Activated")
    ...
    def configure_message_speed(self):
        ...
        if dialog.exec():
            ...
            self.chat_timer.setInterval(int(MESSAGE_RATE * 1000))
            self.add_message(f"System: Message speed adjusted to {MESSAGE_RATE} seconds")
    ...
    def configure_key_mapping(self):
        ...
        if dialog.exec():
            ...
            commands_text = ", ".join([config["command"] for config in self.command_config.values() if config["enabled"]])
    ...
    def check_pause_key(self):
        ...
        try:
            if keyboard.is_pressed(PAUSE_TOGGLE_KEY):
                ...
                icon = "⏸️" if PAUSE_COMMANDS else "▶️"
    ...
    def process_youtube_chat(self):
        ...
        try:
            ...
            if not self.simulation_mode:
                self.connection_label.setText("🔴 YouTube error, simulation mode")
    ...
    def process_twitch_chat(self):
        try:
            ...
            if not self.simulation_mode:
                self.connection_label.setText("🔴 Error on Twitch, simulation mode")
                self.connection_label.setStyleSheet("color: #FF0000;")
                self.add_message("System: ❌ Error on Twitch, activating simulation mode")
                self.simulation_mode = True
                self.simulate_donations()
    ...
    def setup_ui(self):
        ...
        self.connection_label = QLabel("🟡 Getting started...")
        ...
        self.pause_label = QLabel("▶️ Commands ACTIVATED (F12)")
        ...
        donation_title = QLabel("💎 Recent Donations")
    ...
    def init_pytchat(self):
        try:
            self.connection_label.setText("🟡 Connecting to YouTube...")
            ...
            self.connection_label.setText("🟢 Connected to YouTube Live!")
            self.connection_label.setStyleSheet("color: #32CD32;")
            
            self.add_message("System: ✅ Successful connection with YouTube")
            self.add_message("System: 💎 Super Chats activated!")
            
        except Exception as e:
            error_msg = f"Error: {str(e)}"
            self.connection_label.setText("🔴 Connection error")
            self.connection_label.setStyleSheet("color: #FF0000;")
            self.add_message(f"System: ❌ {error_msg}")
            self.add_message("System: 🎮 Activating simulation mode")
            self.simulation_mode = True
            self.simulate_donations()

    def init_twitch(self):
        try:
            self.connection_label.setText("🟡 Connecting to Twitch...")
            self.twitch_connection = TwitchConnection(self.twitch_channel)
            
            time.sleep(1) 
            
            self.connection_label.setText("🟢 Connected to Twitch!")
            self.connection_label.setStyleSheet("color: #32CD32;")
            
            self.add_message("System: ✅ Successful connection to Twitch")
            self.add_message("System: 💎 Bits/donations activated!")
            
        except Exception as e:
            error_msg = f"Error: {str(e)}"
            self.connection_label.setText("🔴 Twitch connection error")
            self.connection_label.setStyleSheet("color: #FF0000;")
            self.add_message(f"System: ❌ {error_msg}")
            self.add_message("System: 🎮 Activating simulation mode")
            self.simulation_mode = True
    ...
    def process_superchat(self, superchat):
        try:
            ...
            donation_text = f"💰 {user} donated ${amount} {currency} → x{multiplier}!"
    ...
    def process_normal_message(self, message):
        self.current_command = f"📩 {message}"
    ...
    def simulate_donation(self, amount, currency, message):
        if self.simulation_mode:
            ...
            self.donations.append(donation_info)
            self.total_donated += usd_amount
            donation_text = f"💰 {donation_info['user']} donó {amount} {currency} → x{multiplier}!"
            self.add_message(donation_text, is_donation=True)
            if message.strip():
                self.apply_donation_effects(message, multiplier)
            self.update_donation_display()
    ...
    def update_simulation(self):
        enabled_commands = [config["command"] for config in self.command_config.values() if config["enabled"]]
        test_messages = [
            ...
            "CoolStreamer: I've been watching the stream for 2 hours 👍",
...
if __name__ == "__main__":
    ...
    if setup_dialog.exec():
        ...
        if platform == "youtube":
            print(f"🚀 Getting started with YouTube ID: {video_id}")
        else:
            print(f"🚀 Getting Started with Twitch Channel: {twitch_channel}")
