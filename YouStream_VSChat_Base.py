...
import pyautogui
import YouStream_VSChat_Connection
from YouStream_VSChat_KeyCodes import *
import time
import threading
import sys

#------------------- SOFTWARE VARIABLES ---------------------# Variables Opcion 4 y 8 Opciones Avanzadas

# Replace this with your Twitch username. Must be all lowercase.
TWITCH_CHANNEL = 'matiasrn1' 
...
YOUTUBE_CHANNEL_ID = "UCEMm41B3zmjmnQaZDTmoG" 
...
current_command = "Esperando comandos..."
messages = []
users = {}
total_messages = 0
votes = {"Opción A": 0, "Opción B": 0, "Opción C": 0}
...
if STREAMING_ON_TWITCH:
    t = YouStream_VSChat_Connection.Twitch()
    t.twitch_connect(TWITCH_CHANNEL)
else:
    t = YouStream_VSChat_Connection.YouTube()
...
def handle_message(message):
    try:
        ...
        global current_command, messages, users, total_messages
        current_command = f"{username}: {msg}"
        messages.append(current_command)
        if len(messages) > 10:
            messages.pop(0)
...
# Función para obtener datos de la interfaz
